...
function ensure_server_dir(hexo) {
  ...
  const dir = path.dirname(get_server_file(hexo));
  fs.mkdirsSync(dir);
...
function emacs_server_start(hexo) {
  ...
  if (!fs.existsSync(init_el)) { init_el = path.join(process.cwd(), 'node_modules', 'hexo-renderer-org', 'emacs', 'hexo-renderer-org.el'); }

  init_el = fix_filepath(init_el);

  const debug_file = tmp.fileSync();

  // convert user_config to absolute path
  let user_config = '';
  if (config.org.user_config) { user_config = path.join(process.cwd(), path.normalize(config.org.user_config)); }

  // Ensure server-dir exist
  if (!ensure_server_dir(hexo)) {
    if (config.org.debug) { console.log('Warning, directory to emacs server-file doesn\'t exist.'); }
...
`;
  ...
  const exec_args = ['-Q', '--daemon=' + get_server_name(hexo), '--eval', emacs_lisp];

  const proc = child_process.spawn(config.org.emacs, exec_args, {
    stdio: 'inherit' // emacs's htmlize package need tty
  });

  proc.on('exit', code => {
    try {
      const oops = JSON.parse(fs.readFileSync(debug_file.name, 'utf8'));
...
function emacs_server_stop(hexo) {
  return new Promise((resolve, reject) => {
    ...
    if (emacs_server_is_dead) { return resolve(); }

    if (!use_emacs_server) { return resolve(); }

    const proc = child_process.spawn(emacsclient, ['-s', get_server_name(hexo), '-e', '(kill-emacs)'], {
      // detached: true
    });
    ...
    proc.on('exit', code => {
      if (code != 0) {
        ...
        setTimeout(() => {
          if (config.org.debug) { console.log('Wait for emacs daemon exit!!'); }
...
function emacs_server_wait(hexo) {
  return new Promise((resolve, reject) => {
    ...
    const proc = child_process.spawn(emacsclient, ['-s', get_server_name(hexo), '-e', '(message "ping")'], {
    });
    ...
    proc.on('exit', code => {
      if (code != 0) {
        ...
        // What timeout interval is better, or spinning with no timeout ?
        setTimeout(() => {
          if (config.org.debug) { console.log('Wait for emacs daemon startup!!'); }
...
function emacs_client(hexo, data, callback) {
  const config = hexo.config;
  if (emacs_server_is_dead) { return; }

  config.highlight = config.highlight || {};

  const emacs_path = config.org.emacs;

  const output_file = tmp.fileSync();
...
`;
  ...
  const exec_args = ['-s', get_server_name(hexo), '-e', emacs_lisp];
  ...
  operation.attempt(currentAttempt => {
    ...
    const proc = child_process.spawn(config.org.emacsclient, exec_args, {
      stdio: 'inherit'
    });

    function retryOrExit(err) {
      if (config.org.debug) { console.log('RETRY: ', data.path); }

      if (emacs_server_is_dead) { callback(''); }

      const retrying = operation.retry(err);
      if (!retrying) {

        if (config.org.debug) { console.log('DONE: ', data.path); }

        const result = fs.readFileSync(output_file.name, 'utf8');
...
`;
  ...
  const exec_args = ['--batch', '--eval', emacs_lisp];
