...
let server_name = 'hexo-renderer-org';

// NOTE: Maybe make this variable configurable ?
// Directory to place server socket file
let server_dir = './emacs.d/server';

// Emacs daemon server-file (full path)
let server_file = undefined;
...
function update_server_file(hexo) {
    ...
    let cachedir = config.org.cachedir;
...
function ensure_server_dir(hexo) {
    ...
    let dir = path.dirname(get_server_file(hexo));
    fs.mkdirsSync(dir);
