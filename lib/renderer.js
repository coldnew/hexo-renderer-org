...
function convert(data, config) {
  return new Promise((resolve, reject) => {
    ...
    var init_el = path.join(process.env.PWD, "emacs", "init.el" );
    if (!fs.existsSync(init_el))
      init_el = path.join(process.env.PWD, "node_modules", "hexo-renderer-org", "emacs", "init.el" );

    var tmpobj = tmp.fileSync();

    var enable_htmlize = !config.highlight.enable;

    // convert user_config to absolute path
    var user_config = "";
    if (config.org.user_config)
      user_config = path.join(process.env.PWD, path.normalize(config.org.user_config));

    // prevent 'undefined' value
    var cachedir = config.org.cachedir || "";
    var theme = config.org.theme || "";

    var emacs_lisp = `

    (let ((args '(:file         "${data.path}"
                  :cache-dir    "${cachedir}"
                  :output-file  "${tmpobj.name}"
                  :htmlize      "${enable_htmlize}"
                  :theme        "${theme}"
...
`;
    ...
    // remove trailing garbage to prevent emacs load fail
