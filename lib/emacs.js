...
function emacs_server_load_config(hexo) {
  ...
  let emacs_lisp = format_hexo_configs(config, config.org.user_config);
...
function emacs_client(hexo, data) {
  ...
  if(batch_station === null || batch_station.status === STOPED) {
    // start batch station if it is null
    batch_station = new BatchStation(config.org.emacsclient, {debug: config.org.debug});
