...
function read_info(data) {
  ...
  function read_tags(){
    if(_items.TAGS){
      data.setTags(_items.TAGS.split(/[\s,]+/).map((item) => {
        // convert tags like 'raspberry_pi' to 'raspberry pi'
        item = item.replace(/_/g, ' ');
        // convert tags like 'org@mode' to 'org-mode'
        item = item.replace(/@/g, '-');
        item.trim();
        return item;
      }));
  ...
  function read_categories(){
    if(_items.CATEGORIES){
      data.setCategories(_items.CATEGORIES.split('/[\s,]+/').map((item) => {
        // convert category like 'raspberry_pi' to 'raspberry pi'
        item = item.replace(/_/g, ' ');
        // convert category like 'org@mode' to 'org-mode'
        item = item.replace(/@/g, '-');
        item.trim();
        return item;
      }));
