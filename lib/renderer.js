...
(progn
  ...
  (setq *debug-file* "${fix_filepath(debug_file.name)}")
  ;; load init.el
  (load "${fix_filepath(init_el)}")
  ;; render file according to args
  (hexo-render-org '(:file         "${fix_filepath(data.path)}"
                     :cache-dir    "${fix_filepath(config.org.cachedir) || ""}"
                     ...
                     :common       "${config.org.common.replace(/\n/g, "\\n")}"
                     )))
`;
